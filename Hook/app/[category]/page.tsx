        ...
        <div className="p-5 w-full flex flex-col h-screen ">
            ...
            {shownCat.length !== 0 && shownCat.items?.map((item: any) => (
                <Link key={item.name} href={`/${resolvedParams?.category}/${item.name}`}>
                    <button className="w-[300px] h-[100px] mb-5 px-5 py-3 bg-blue-950 text-white hover:bg-white hover:outline-3 hover:outline font-bold outline-orange-500 outline-offset-2 hover:text-black transition-all">
                        {item.name}
                    </button>
                </Link>
            ))}
